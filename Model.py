...
from tick_data_generator import genEpisode, df_trading_day, upperBound, mu_time, std_time, mu_bid, std_bid,partitionTradeDay,genTradingDay
...
class ExecutionAgent:
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            print('Starting execute minute')
            
            test_files = ['AAPL_20180117.gz','FB_20180117.gz','MSFT_20180117.gz']
            
            for test_file in test_files:
                print('Starting testing for :'+str(test_file))
                df_trading_day, upperBound, mu_bid, mu_time, std_bid, std_time = genTradingDay(test_file)
                ...
                for ite in range(self.exec_iterations):
                    ...
                    grad = False
                    while not done:
                        prob = self.actor.compute_prob(np.expand_dims(obs, 0))
                        if grad:
                ...
                plt.legend()
                plt.savefig('tradeStats'+test_file+'.png')
                plt.close()
                ...
                ax[4].legend()
                plt.savefig('Probability'+test_file+'.png')
                plt.close()
                ...
                ax[4].legend()
                plt.savefig('outputProbability'+test_file+'.png')
                plt.close()
